...
Proof.
   ...
   intros P Q.
  split. intros.
  inversion H as [x0 H0].
  inversion H0 as [Hl | Hr].
    left. exists x0. apply Hl.
    right. exists x0. apply Hr.

  intros.
  induction H as [Hl | Hr].
  inversion Hl.
...
Fixpoint All {T} (P : T -> Prop) (l : list T) : Prop :=
    match l with
  | [] => True (*и єто логично, ведь пустое множество может біть частью property*)
  | x :: l' => P x /\ All P l' 
  end.
...
Proof.
  ...
  + simpl. intros. split. apply H. left. reflexivity.
  ...
  + intros H.  induction l.
    ...
    - simpl. intros. destruct H0 as [H1 | H1]. destruct H as [H2 H3]. rewrite <- H1. assumption.
      destruct H as [H2 H3]. apply IHl. assumption. assumption.
Qed.
...
(*Proof.
  ...
  intros. induction x as [|x l1 IH]. simpl. reflexivity.
  simpl. rewrite <- IH. induction l1. 
  simpl. reflexivity. simpl. destruct IHl1. induction l1. simpl. reflexivity. 
simpl. apply app_assoc.
unfold tr_rev. induction l1.
*) Admitted.
